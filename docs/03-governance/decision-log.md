...
| D-040 | 2026-09-23 | D-039 의 AH371 판번호를 정정한다. 현행판은 **KGS AH371 2025**(2025.12.9 기후에너지환경부 승인)이며 앞서 기록한 '24.7.23. 판은 구판이다. **비적용 판정은 유지한다** — 적용범위 1.1.1 의 232.6 ㎾ 상한이 두 판에서 동일하다. 20 MW 급 적용 후보로 KGS FU111·FP111·FP112·FU671·FP216·FP217 6건을 등록하되 전부 `undetermined` 로 둔다. | 현행판 원문 79쪽을 받아 표제지·제개정이력·적용범위를 직접 대조했다. 후보 6건은 목록에서 제목·개정일만 확인했고 적용범위 조문은 보지 않았으므로 확정하지 않는다. | 가스기술기준 승인 기관이 산업통상자원부에서 기후에너지환경부로 변경된 것을 확인했다. h2hub 게시물 메타데이터가 원문 공고번호와 어긋나는 사례도 확인했다 — 포털이 아니라 원문을 근거로 삼는다. **서지 확인 결과는 유효기간이 있다. 인용 전 현행판 재확인이 필요하다.** | Chief Engineer + HSE/Regulatory | Approved |
