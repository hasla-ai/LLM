...
| F-02 | mission-catalog.md 가 스키마 필수 필드를 갖고 있지 않다. `mission-content-draft.json`에 목적·완료기준·차단조건 100/100 작성됨(D-030). 생성 정의는 스키마 100/100이나 입력·출력 `schema_ref`와 `quality_checks`가 남아 있어 전체 계약은 미완성이다. 선행관계 101건 확정(D-028·D-029). `mission-library-100.json`은 이식 원본이 아니다(D-025). | — |
